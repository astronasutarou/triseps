...
class wcs_directory(object):
    ...
    def __enter__(self):
        ...
        if __DEBUG__:
            eprint(f'{self.dirname} created')
        os.chdir(self.dirname)
        if __DEBUG__:
            eprint(f'change directory to {self.dirname}')

    def __exit__(self, type, value, traceback):
        os.chdir(self.cwd)
        if __DEBUG__:
            eprint(f'change directory to {self.cwd}')
        if not __DEBUG__:
...
def wcspaste(src, dst):
    ...
    for key in keywords:
        dst.header[key] = src.header[key]
    if 'SIP' in src.header['CTYPE1']:
        for key in sipA_keywords:
            dst.header[key] = src.header.get(key, 0.0)
    if 'SIP' in src.header['CTYPE2']:
        for key in sipB_keywords:
            dst.header[key] = src.header.get(key, 0.0)
